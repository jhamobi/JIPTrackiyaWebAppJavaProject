...
public interface VehicleServiceAsync {
	...
	void fetchAllVehiclesWithTrackingData(AsyncCallback<List<Vehicle>> callback);

	void fetchAllVehiclesWithDetails(AsyncCallback<List<Vehicle>> callback);

	void fetchVehiclesWithDetails(String hubId,
			AsyncCallback<List<Vehicle>> callback);

	void getVehicleDetail(String vehicleId, AsyncCallback<Vehicle> callback);
