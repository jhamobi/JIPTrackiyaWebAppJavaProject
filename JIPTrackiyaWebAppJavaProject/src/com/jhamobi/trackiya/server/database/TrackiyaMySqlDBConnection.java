...
public class TrackiyaMySqlDBConnection {
	...
	private static String MYSQL_URL = "jdbc:mysql://http://yufondo-db.c1xosa2sqoup.ap-south-1.rds.amazonaws.com/jhamobi_trackiya";
	
	private static String MYSQL_USERNAME = "admin";
	private static String MYSQL_PASSWORD = "Yofundo01$";
	...
	public void insertReportTime(Connection connection, String vehicleId, String time) throws SQLException{
		...
		ResultSet rs = null;
		String columns = " ("+TrackiyaDBTableColumnNames.VEHICLE_ID+","+ TrackiyaDBTableColumnNames.DATE +","+TrackiyaDBTableColumnNames.REPORT_TIME +")";
		String values = " values(?,?,?)";
		String statement = "INSERT INTO " + TrackiyaDBTableColumnNames.VEHICLE_RUNNING_DATA_TABLE +columns+ values;
		PreparedStatement getStatement =  connection.prepareStatement(statement);
		try {
	...
	public void insertStartTime(Connection connection, String vehicleId, String time) throws SQLException{
		...
		try {
			...
			getStatement.setString(2, vehicleId);
			getStatement.execute();
		} finally{
			if(rs != null)
				rs.close();
	...
	public void updateRunningData(Connection connection, String vehicleId, RunningData data) throws SQLException{
		...
		try {
			...
			getStatement.setString(9, vehicleId);
			getStatement.execute();
		} finally{
			if(rs != null)
				rs.close();
	...
	public void insertLongitudeAndLatitude(Connection connection, String vehicleId, double longitude, double latitude) throws SQLException{
		...
		try {
			...
			getStatement.setString(3, vehicleId);
			getStatement.execute();
		} finally{
			if(rs != null)
				rs.close();
